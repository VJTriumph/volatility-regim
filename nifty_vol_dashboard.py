...
<div id="charts">
  ...
  <div class="chart-card"><h3>VIX Regime · Expiry Vol</h3>
  <div id="vix-toggle" style="margin-bottom:6px;display:flex;gap:4px">
    <button class="tab on" onclick="setVixView(this,'both')">Both</button>
    <button class="tab" onclick="setVixView(this,'weekly')">Weekly Expiry</button>
    <button class="tab" onclick="setVixView(this,'monthly')">Monthly Expiry</button>
  </div>
  <div id="c-vix" style="height:260px"></div></div>
</div>
...
let state = {{
  ...
  outlier:   2.5,       // sigma, 0=off
    ...
    vixView:   "both",    // both|weekly|monthly
  dateStart: "{all_start}",
  dateEnd:   "{all_end}",
}};
...
// VIX regime chart
function analyzeVixRegimes(rows,estimator,vixLow,vixHigh,view){{
  ...
  for(const reg of["Low","Med","High"]){{
    ...
    const f=view==="monthly"?"meg":"weg";
    const expGrp={{
      DayBefore:sub.filter(r=>r[f]==="DayBefore"),
      ExpiryDay:sub.filter(r=>r[f]==="ExpiryDay"),
      DayAfter:sub.filter(r=>r[f]==="DayAfter"),
      Other:sub.filter(r=>r[f]==="")
    }};
    const vals={{}};
...
function setVixView(btn,v){{
  state.vixView=v;
  document.querySelectorAll('#vix-toggle .tab').forEach(b=>b.classList.remove('on'));
  btn.classList.add('on');
  renderVixChart(analyzeVixRegimes(getFiltered(),state.estimator,state.vixLow,state.vixHigh,v));
}}
...
function update(){{
  ...
  renderVixChart(analyzeVixRegimes(rows,state.estimator,state.vixLow,state.vixHigh,state.vixView));
