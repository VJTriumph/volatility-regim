...
function skewness(a){{
  ...
  const mu=mean(a);
  const sd=Math.sqrt(sampleVar(a));
...
function kurtosis(a){{
  ...
  const sd=Math.sqrt(sampleVar(a));
  if(sd<1e-12) return 0;
  return a.reduce((s,v)=>s+((v-mu)/sd)**4,0)/n - 3;
}}
...
function lag1Acf(a){{
  ...
  const mu=mean(a);
  let num=0, den=0;
  for(let i=1;i<n;i++) num+=(a[i]-mu)*(a[i-1]-mu);
  for(let i=0;i<n;i++) den+=(a[i]-mu)**2;
  return den<1e-12 ? 0 : num/den;
}}

function cohensD(g,r){{
  if(g.length<2||r.length<2) return 0;
  const ag=g.map(Math.abs), ar=r.map(Math.abs);
  const n1=ag.length, n2=ar.length;
  const pooledSD=Math.sqrt(((n1-1)*sampleVar(ag)+(n2-1)*sampleVar(ar))/(n1+n2-2));
  return pooledSD<1e-12 ? 0 : (mean(ag)-mean(ar))/pooledSD;
}}

function permutationTest(g,r,nPerm=1000){{
  const ag=g.map(Math.abs), ar=r.map(Math.abs);
  if(ag.length<3||ar.length<3) return 1;
  const n1=ag.length;
  const obs=Math.abs(mean(ag)-mean(ar));
  const pool=[...ag,...ar];
  const N=pool.length;
  let cnt=0;
  for(let i=0;i<nPerm;i++){{
    ...
    const g1=s.slice(0,n1);
    const rest2=s.slice(n1);
    const diff=Math.abs(mean(g1)-mean(rest2));
    if(diff>=obs) cnt++;
  }}
  return (cnt+1)/(nPerm+1);
}}

function bootstrap(vals, estimator, nIter=1000){{
  const n=vals.length;
...
function analyzeGroup(rows, keyFn, labels, estimator){{
  ...
  for(const lbl of labels){{
    ...
    if(grp.length<10){{
      results.push({{label:String(lbl),n:grp.length,vol:NaN,ciLo:NaN,ciHi:NaN,pval:1,adjP:1,ratio:NaN,meanRet:NaN,cohenD:NaN,skew:NaN,acf:NaN,ciOverlapAvg:false,oosVol:NaN,insVol:NaN,periodSig:0,periodTotal:0}});
      continue;
    }}
    const vol=estVol(gVals,estimator);
    const [ciLo,ciHi]=bootstrap(gVals,estimator,1000);
    // Permutation test (distribution-free) on absolute returns
    const pval=permutationTest(grp.map(r=>r.ret),rest.map(r=>r.ret),1000);
    const gRets=grp.map(r=>r.ret);
    const rRets=rest.map(r=>r.ret);
    const d=cohensD(gRets,rRets);
    const sk=skewness(gVals);
    const acf=lag1Acf(gVals);
    const holdN=Math.max(1,Math.floor(gVals.length*0.2));
    const oosVals=gVals.slice(-holdN);
    const insVals=gVals.slice(0,-holdN);
    const oosVol=oosVals.length>=2?estVol(oosVals,estimator):NaN;
    const insVol=insVals.length>=2?estVol(insVals,estimator):NaN;
    const ciOverlapAvg=(!isNaN(ciLo)&&!isNaN(ciHi))?(ciLo>overallVol||ciHi<overallVol):false;
    const mr=mean(gRets);
    results.push({{label:String(lbl),n:grp.length,vol,ciLo,ciHi,pval,adjP:1,ratio:vol/overallVol,meanRet:mr,cohenD:d,skew:sk,acf,ciOverlapAvg,oosVol,insVol}});
  }}
  ...
  // Period stability & OOS consistency
  results.forEach(r=>{{
    const ps=periodStability(rows,keyFn,r.label,estimator);
    r.periodSig=ps.sigCount;
    r.periodTotal=ps.validCount;
  }});

  return {{results,overallVol}};
}}

// ── Period Stability: run analyzeGroup across 3 known periods ─────────────
function periodStability(allRows, keyFn, label, estimator){{
  const periods=[
    {{s:'2015-01-01',e:'2019-12-31'}},
    {{s:'2020-01-01',e:'2024-10-31'}},
    {{s:'2024-11-01',e:'2099-12-31'}}
  ];
  let sigCount=0, validCount=0;
  for(const per of periods){{
    const pRows=allRows.filter(r=>r.date>=per.s&&r.date<=per.e);
    if(pRows.length<20) continue;
    const gRows=pRows.filter(r=>keyFn(r)===label);
    const rRows=pRows.filter(r=>keyFn(r)!==label);
    if(gRows.length<5||rRows.length<5) continue;
    const p=permutationTest(gRows.map(r=>r.ret),rRows.map(r=>r.ret),200);
    validCount++;
    if(p<0.05) sigCount++;
  }}
  return {{sigCount,validCount}};
}}

function analyzeAll(rows,estimator){{
  // Day of month — only keep days with n>=10
  const domAll=[...Array(31)].map((_,i)=>i+1);
  const domResult=analyzeGroup(rows,r=>r.dom,domAll,estimator);
  domResult.results=domResult.results.filter(r=>r.n>=10);
...
function renderBar(divId,analysis){{
  ...
  const hover=results.map(r=>`
    ...
    Cohen's d: ${{isNaN(r.cohenD)?"\u2013":r.cohenD.toFixed(2)}}<br>
    Skew: ${{isNaN(r.skew)?"\u2013":r.skew.toFixed(2)}}<br>
    ACF(1): ${{isNaN(r.acf)?"\u2013":r.acf.toFixed(2)}}<br>
    Periods sig: ${{r.periodTotal>0?r.periodSig+"/"+r.periodTotal:"\u2013"}}<br>
...
function renderSummary(rows,analysis){{
  ...
  const items=[
    ...
    {{label:"Skewness",val:skewness(rets).toFixed(2),cls:Math.abs(skewness(rets))>1?"hi":"lo"}},
    {{label:"Kurtosis",val:kurtosis(rets).toFixed(2),cls:kurtosis(rets)>3?"hi":"lo"}},
...
function renderTable(analysis){{
  ...
  const thead=`<tr>
    ...
    <th>FDR-adj p</th><th>Signal</th>
    <th title="Cohen's d">d</th><th title="Skewness">Skew</th>
    <th title="Lag-1 ACF">ACF</th><th title="OOS Vol / In-sample Vol">OOS</th>
    <th title="Significant in X/3 periods">Stability</th>
    <th title="Reliability score 0-5">★</th>
  </tr>`;
  ...
  const tbody=allRows.map(r=>{{
    ...
    const dVal=isNaN(r.cohenD)?"\u2013":r.cohenD.toFixed(2);
    const dCls=Math.abs(r.cohenD||0)>0.5?"sig-hi":Math.abs(r.cohenD||0)>0.2?"sig-med":"";
    const skVal=isNaN(r.skew)?"\u2013":r.skew.toFixed(2);
    const acfVal=isNaN(r.acf)?"\u2013":r.acf.toFixed(2);
    const acfFlag=Math.abs(r.acf||0)>0.15?"\u26a0":"";
    const oosRatio=(!isNaN(r.oosVol)&&!isNaN(r.insVol)&&r.insVol>0)?(r.oosVol/r.insVol):NaN;
    const oosStr=isNaN(oosRatio)?"\u2013":((oosRatio-1)*100).toFixed(0)+"%";
    const oosCls=Math.abs((oosRatio||1)-1)<0.3?"sig-hi":"sig-med";
    const psStr=(r.periodTotal>0)?`${{r.periodSig}}/${{r.periodTotal}}`:"\u2013";
    const psCls=(r.periodSig>=2&&r.periodTotal>=2)?"sig-hi":(r.periodSig>=1&&r.periodTotal>=2)?"sig-med":"";
    let score=0;
    if(r.adjP<0.05) score++;
    if(Math.abs(r.cohenD||0)>0.2) score++;
    if(r.ciOverlapAvg) score++;
    if(r.periodSig>=2) score++;
    if(!isNaN(oosRatio)&&Math.abs(oosRatio-1)<0.3) score++;
    const stars="\u2605".repeat(score)+"\u2606".repeat(5-score);
    const starCls=score>=4?"sig-hi":score>=2?"sig-med":"";
    return `<tr>
      ...
      <td>${{skVal}}</td>
      <td>${{acfVal}}${{acfFlag}}</td>
      <td class="${{oosCls}}">${{oosStr}}</td>
      <td class="${{psCls}}">${{psStr}}</td>
      <td class="${{starCls}}" style="letter-spacing:-1px">${{stars}}</td>
    </tr>`;
  }}).join("");
...
function renderNotice(rows){{
  ...
  document.getElementById("notice").textContent=
    `Estimator: ${{est}}  ·  Bootstrap CIs: 1000 iterations · Permutation test (distribution-free) · n≥10 filter  ·  `+
    `FDR correction: Benjamini-Hochberg  ·  `+
    `${{nSig5.length}} group(s) significant at FDR-adjusted p<0.05  ·  `+
    `Reliability ★★★★★ = p<0.05 + |d|>0.2 + CI excl avg + 2/3 periods + OOS stable`;
}}
